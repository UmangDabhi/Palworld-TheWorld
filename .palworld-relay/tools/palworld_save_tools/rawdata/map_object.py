    ...
    for map_object in value["value"]["values"]:
        ...
        for module in map_object["ConcreteModel"]["value"]["ModuleMap"]["value"]:
            ...
            try:
                ...
                if "EOF not reached" not in str(exc):
                    raise
                # Palworld frequently adds raw-data module fields before the
                # community parsers know how to decode them. Preserve the
                # original bytes so unrelated save data round-trips unchanged.
                module["value"]["RawData"]["value"] = {"values": module_bytes}
    return value
